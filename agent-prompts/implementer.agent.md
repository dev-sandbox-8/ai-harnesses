...
(`agent-output/<id>.md`): list every file changed, every test added or modified, and
any blocked items with diagnostics.
...
4. **Maintain architectural rules.** Every change must comply with
   `AGENTS.md` (RSC boundaries, design-token-only styling, mobile-first
...
ONLY when given a path to a spec file in `specs/`:
...
ONLY when given a structured fix-list (from code-reviewer, quality-gate, or refactor agent):
