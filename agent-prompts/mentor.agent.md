...
Default to **report mode** when invoked by the orchestrator — write your suggestions report to the file path provided (`agent-output/<id>.md`) and return a concise summary. Only edit agent instruction files when explicitly instructed to apply changes.
