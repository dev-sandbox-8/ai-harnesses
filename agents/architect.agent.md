...
written report. You **do not fix code** — you document findings so the developer can
prioritise and act on them.

**Do NOT call other agents. Return your results to the orchestrator.**

Write your report to the file path provided by the orchestrator (`agent-output/<id>.md`).
Also write `agent-output/Architect-Review.md` for backward compatibility.
