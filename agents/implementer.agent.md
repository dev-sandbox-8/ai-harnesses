...
3. **Incremental verification.** Run all tests (unit, lint, e2e) after each meaningful     
   change to catch regressions early. Do not wait until the end to discover failures.
...
5. Run all tests (unit, lint, e2e) to verify.
...
After all items are complete (or blocked), run the full test suite (unit, lint, e2e) one final time.
